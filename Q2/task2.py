...
def handle_moveto(robot, y, x, features):
    target = Location(y, x)
    if robot.current_location == target:
        print("same location")
        return 0
    
    path = robot.calculate_path(target)
    days = len(path) - 1
    robot.log_move(path, days)
    return days

def handle_explore(robot, features):
    ...
    print(f"explore {feature}")
    robot.log_exploration(feature, days)
    return days

# def main():
#     # Existing task1 setup
